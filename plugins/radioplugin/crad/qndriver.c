...
int get_ch_start_for_country(int country) {
	if(country == COUNTRY_EUROPE)
		return 8700;
...
int get_ch_stop_for_country(int country) {
	if(country == COUNTRY_EUROPE)
		return 10800;
	if(country == COUNTRY_ITALY)
		return 10800;
...
int get_ch_step_for_country(int country) {
	if(country == COUNTRY_EUROPE)
		return 1;
	if(country == COUNTRY_ITALY)
		return 0;
...
{
	qnd_Country = country;
	QND_CH_START = get_ch_start_for_country(country);
	QND_CH_STOP  = get_ch_stop_for_country(country);
	QND_CH_STEP  = get_ch_step_for_country(country);
}
